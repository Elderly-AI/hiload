...
* Медиа статика
	...
	* Рассмотрим так же оставшиеся критерии (CPS и пропускную способность)  для сервера с конфигурацией:
		...
		|          CPU         | Memory  |    SSD    |
		|----------------------|---------|-----------|
		|  2x Intel(R) 8 cores | 256 GB  |  2TB x 64 |
			
	* Исходя из документации nginx [1.](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/) 
[2.](https://www.nginx.com/blog/nginx-websockets-performance/) видно, что CPS для данного сервера ~10 000. Тогда CPS для 394 серверов равен 394 * 10 000 = 3 940 000CPS > 750 000, следовательно проблем с CPS нет. Пропускная способность для такого сервера ~70GBit, тогда общая пропускная способность 394 * 70Gbit = 27Tbit > 14Tbit * 1.2, следовательно с пропускной способностью всё в порядке. Таким образом получаем 394 сервера для отдачи медиа данных с сипользованием технологии RAID 10
...
* Итог
	...
	|	        | 2x Intel(R) 8 cores  |  256GB   | 1TB x 64 |   990  | 1170  | 
...
* Расчитаем стоимость использования облачного сервиса Amazon Web Services. [1.](https://aws.amazon.com/ru/about-aws/global-infrastructure/)
	...
	* Произведём расчёт стоимости, опираясь на [1.](https://aws.amazon.com/ru/ec2/pricing/on-demand/) и [2.](https://aws.amazon.com/ru/ebs/pricing/). При расчёте, при отсутствии требуемой конфигурации бралась ближайшая по аппаратной конфигурации.
	...
	|	        | 2x Intel(R) 8 cores  |  256GB   | 1TB x 64 |   990  | 1,144 USD/h x 990  | 10,235 USD/h x 990 | 11265 USD/h | 11 395 USD/h |
	...
	| Фронтенд      | 11395 USD/h  |             |
	| Бэкенд        | 3964,8 USD/h | 15585 USD/h |

* Примерно расчитаем стоимость железа:

	|	        | Стоимость       | Сумма         | Итог          |
	|---------------|-----------------|---------------|---------------|
	| SSD 128GB     | 18 USD x 471    | 8 478 USD     |               |
	| SSD 256GB     | 27 USD x 180    | 4 860 USD     |               |
	| SSD 512GB     | 53 USD x 744    | 39 432 USD    |               |
	| SSD 1TB       | 80 USD x 63 360 | 5 068 800 USD |               |
	| SSD 4TB       | 430 USD x 5760  | 2 476 800 USD | 7 598 370 USD |
	
	|	                          | Стоимость       | Сумма         | Итог          |
	|---------------------------------|-----------------|---------------|---------------|
	| 2 x Intel(R) Xeon(R) v3 36 core | 2616 USD x 843  | 2 205 288 USD |               |
	| 2 x Intel(R) Xeon(R) v3 8 core  | 1000 USD x 990  | 990 000 USD   | 3 195 288 USD |

	|	        | Стоимость       | Сумма             | Итог          |
	|---------------|-----------------|-------------------|---------------|
	| RAM 16GB      | 60 USD x 331    | 19 860 USD        |               |
	| RAM 32GB      | 208 USD x 324   | 67 392 USD        |               |
	| RAM 128GB     | 832 USD x 71    | 59 072 USD        |               |
	| RAM 256GB     | 1664 USD x 1035 | 1 722 240 880 USD | 1 868 564 USD |
	
	|	        | Сумма         | Итог           |
	|---------------|---------------|----------------|
	| SSD           | 7 598 370 USD |                |
	| RAM           | 1 868 564 USD |                |
	| Processors    | 3 195 288 USD | 12 662 222 USD |
...
* Оценим время окупаемости собственных серверов и датацентров:
	(12 662 222 USD + 3 666 000 USD) / 15 558 USD/h = 1049h или 43 дня. )
	
* Выводы:
	На много дешевле разместить свои датацентры.

### 9. Схема балансировки нагрузки (входящего трафика и внутрипроектного, терминация SSL)
* Будем использовать nginx для балансировки нагрузки с использованием схемы L7. Это позволит равномерно распределить нагрузку и решит проблемы медленных клиентов. Также nginx имеет внутренние инструменты для настройки SSL терминации. Внутрипроектный трафик также будет балансироваться при помощи L7 балансировщика. 
