...
[2.](https://www.nginx.com/blog/nginx-websockets-performance/), можно сделать вывод, что для балансировки и терминации ssl ~ 8 400 000RPS, для лёгких запросов понадобится 8 400 000RPS / 90 000RPS = 94 сервера конфигурации:
			...
			|          CPU          |  Memory |   SSD   |
			|-----------------------|---------|---------|
			|  2x Intel(R) 36 cores |  16 GB  |  256GB  |
...
* Бэкенд.
	...
	* Сервис авторизации
		...
		* Оценивая средний RPS на ядро для redis в 10 000RPS получим одну машину, которой будет достаточно, для отказоустойчивости возьмём сумарно 3 машины конфигурации:
	...
	* Итог
		...
		|	        |          CPU         |  Memory  |   SSD    |
		|---------------|----------------------|----------|----------|
		|    Фронтенд   | 2x Intel(R) 32 cores |  32 GB   | 128GB    |
		|	        | 2x Intel(R) 32 cores |  32 GB   | 128GB    |
		|	        | 2x Intel(R) 32 cores |  32 GB   | 128GB    |
