...
* Бэкенд.
	...
	* Сервис лайков
		...
		* Таким образом исходя из документации nginx [1.](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/) [2.](https://www.nginx.com/blog/nginx-websockets-performance/), что будет достаточно 15 машин с конфигурацией:
	...
	* Сервис тегов
		...
		* Таким образом исходя из документации nginx [1.](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/) [2.](https://www.nginx.com/blog/nginx-websockets-performance/), что будет достаточно 30 машин с конфигурацией:
	...
	* Сервис авторизации
		...
		* Оценивая средний RPS на ядро для redis в 10 000RPS получим одну машину, которой будет достаточно, для отказоустойчивости возьмём сумарно 6 машин конфигурации:
	...
	* Сервис сессий
		...
		* Оценивая максимальную производительность redis базы данных в 10 000 RPS на ядро получим, что потребуется 460 000RPS / 32 / 10 000RPS = 1 видим, что будет достаточно 1 сервера, но для отказоустойчивости вырём в 6 раз больше, потому, что это тонкое место всего проекта. Получаем 6 серверов комплектации: 
	...
	* Итог
		...
		|	        | 2x Intel(R) 32 cores |  32GB    | 512GB x 20 |   6    |       | 
		|	        | 2x Intel(R) 32 cores |  16GB    | 512GB x 32 |   4    |       | 
		|	        | 2x Intel(R) 32 cores |  256GB   | 128GB      |   15   |       | 
		|	        | 2x Intel(R) 32 cores |  32GB    | 4TB x 64   |   30   |       | 
		|	        | 2x Intel(R) 32 cores |  32GB    | 128GB      |   6    |       | 
		|	        | 2x Intel(R) 32 cores |  32GB    | 128GB      |   6    |  138  | 
		
		Сумарно получим 319 + 1170 + 138 = 1627
