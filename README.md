...
* Для балансировки нагрузки будем использовать Anycast DNS Load Balancing. Сформируем 33 пачки по 3 сервера из 3х разных зон доступности, и присвоим им ip адреса. Для каждого пришедшего пользователя будем отдавать ip трёх таких сформированных пачек, при этом для каждого последующего пользователя, будем раздавать такой же список ip адресов, но сдвинутых на 1 ip адрес. Переход к L7 балансировщику для каждой пачки будам так же последовательно распределять для каждого пользователя при обращении. Балансировщики будут проверяться специальным скриптом, и при выходе из строя будут убираться из таблицы маршрутизации. 

	* Преимущества такого подхода  заключаются в: 
	
		* Равномерном распределении нагрузки на L7 балансировщики и на датацентры с учётом кэширования ip адресов на стороне клиента.
		
		* Резервирование L7 балансировщиков
...
* Для отдачи медиа статики nginx будет перенапрявлять пользователя непосредственно на сервер, раздающий видео, такой подход поможет значительно уменьшить исходящий трафик, проходящий через балансировщки. Для определения конкретного сервера для конкретного видео слишком дорого по ресурсам создвать таблицу, храняцую как ключ конкретный id видео и как значение сервера, на которых оно имееется, по этому воспользуемся другим "комбинированным" подходом.

	* Комбинированный подход:
	
		* Будем использовать такую же таблицу, но в ней будем хранить только те id видео, которые были перемещенны.
		
		* Создадим таблицу групп серверов, то есть master-slaves в нашем случае, ключём такой таблицы является номер группы, а знасением соответсвенно их 		адреса. При неисправности сервера, он убирается из таблицы с помощью специального скрипта.
		
		* Когда требуется найти видео, поиск с начала осуществляется в этой таблице, если записи в такой таблице не найденно, то делается следующая 			операция: берём hash от id видео который нужно найти, далее берём остаток от деления hash на колличество master раздающих серверов, полученное 			число - номер группы серверов, где располагается видео. Необходимые сервера берутся из таблицы групп серверов. 
		
	* Преимущества такого подхода:
		
		* Из-за рандомности полученных hash получаем равномерное распределение нагрузки на сервера.
		
		* Можно перемещать некоторые видео.
		
		* Легко можно добавить дополнительные сервера.
		
		* Быстрый поиск необходимого видео, при небольшом колличестве перемещённых видео.

### 10. Обеспечение отказоустойчивости

* Использование собственных датацентров с 3-мя зонами доступности ).
